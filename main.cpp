...
//Complejidad: O(E log E) conviene con un grafo normal, pocos arcos o medianos
void Graph::kruskalMST(){ 
  // Aquí va tu código
  costMSTKruskal = 0;
  sort(edges.begin(), edges.end()); // O(nog2n)
  DisjointSets ds(V);
  for(auto it: edges){
...
//O(V^2) conviene para grafos muy conectados casi completos
void Graph::primMST(){
  // Aquí va tu código
  int selSource =0;
  costMSTPrim = 0;
  ...
  while(conected > 0){
    ...
    //Meter al vector de pares el que fuiste seleccionando
